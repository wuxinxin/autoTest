...
import com.example.dztest.utils.db.rollback.db.Changes;
import com.example.dztest.utils.db.rollback.exception.AnnotationException;
import com.example.dztest.utils.db.rollback.exception.RollBackException;
...
public class RollBackListener extends TestListenerAdapter {
    ...
    static {
        System.out.println("loading RollBackListener now ...");
    }
    ...
    private boolean isRollBackMethod(Method testMethod) throws AnnotationException {
        ...
        if (annotations != null) {
            com.example.dztest.utils.db.rollback.RollBack rollBack = testMethod.getAnnotation(com.example.dztest.utils.db.rollback.RollBack.class);
            com.example.dztest.utils.db.rollback.RollBackAll rollBackAll = testMethod.getAnnotation(com.example.dztest.utils.db.rollback.RollBackAll.class);
            if (rollBack == null) {
                return false;
    ...
    private boolean isRollBackAllMethod(Method method) throws AnnotationException {
        ...
        if (annotations != null) {
            com.example.dztest.utils.db.rollback.RollBack rollBack = method.getAnnotation(com.example.dztest.utils.db.rollback.RollBack.class);
            com.example.dztest.utils.db.rollback.RollBackAll rollBackAll = method.getAnnotation(com.example.dztest.utils.db.rollback.RollBackAll.class);
            if (rollBackAll != null) {
                if (rollBack != null) {
                    Reporter.log("@RollBack 和 @RollBackAll 不能同时使用");
                    throw new AnnotationException("@RollBack 和 @RollBackAll 不能同时使用");
    ...
    private String getDbName(Method testMethod) {
        // 从注解中获取数据库名
        com.example.dztest.utils.db.rollback.RollBack annotation = testMethod.getAnnotation(com.example.dztest.utils.db.rollback.RollBack.class);
